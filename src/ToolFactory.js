...
class ToolFactory {
  ...
  uploadToolsFromJSON() {
    try {
      ...
      Logger.info(
        `Tool upload completed: ${toolsCreated} created, ${toolsUpdated} updated`
      );
  ...
  processToolRecord(record) {
    try {
      ...
      const diameter = this.parseNumericValue(
        record.diameter || record.Diameter
      );
      const toolCode = this.parseNumericValue(
        record.toolCode || record.ToolCode || record.tool_code
      );

      if (diameter === null || toolCode === null) {
        Logger.warn(
          `Skipping record with invalid diameter (${diameter}) or toolCode (${toolCode})`
        );
  ...
  updateToolFromRecord(tool, record) {
    ...
    const currentTime = this.parseNumericValue(
      record.currentTime || record.current_time
    );
    if (currentTime !== null) {
  ...
  bulkCreateTools(toolSpecs) {
    ...
    for (const spec of toolSpecs) {
      try {
        ...
        Logger.error(
          `Failed to create tool D${spec.diameter} P${spec.toolCode}: ${error.message}`
        );
      }
    }
