...
class ToolLogic {
  ...
  findBestToolForJob(diameter, toolCode, requiredTime) {
    try {
      Logger.info(`Finding best tool for: D${diameter} P${toolCode}, time: ${requiredTime}min`);
      
      // Find all tools matching diameter and code
      const matchingTools = Matrix.getAllTools().filter(tool => 
      ...
      if (capableTools.length === 0) {
        return {
          ...
          alternatives: this.findAlternativeTools(diameter, toolCode, requiredTime)
        };
      }
  ...
  findAlternativeTools(diameter, toolCode, requiredTime) {
    ...
    const sameDiameterTools = Matrix.getAllTools().filter(tool => 
      tool.diameter === diameter && tool.toolCode !== toolCode
    );
    ...
    const similarDiameterTools = Matrix.getAllTools().filter(tool => 
  ...
  assignToolToProject(tool, project, cuttingTime) {
    try {
      ...
      Logger.info(`Assigned tool ${tool.toString()} to project ${project.getFullId()}`);
  ...
  getToolUtilizationStats() {
    ...
    for (const tool of allTools) {
      const utilization = tool.maxTime > 0 ? (tool.currentTime / tool.maxTime) * 100 : 0;
    ...
    const nonMaxedTools = utilizationData.filter(data => data.state !== ToolState.MAXED);
